...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    new = np.zeros((num_rows, num_cols))
    for i in range(num_rows):
        for j in range(num_cols):
            if gray_img[i,j] >= lower_thresh and gray_img[i,j] <= upper_thresh:
                new[i,j] = 1
            else:
                new[i,j] = 0
    return new
...
def edge_detect_naive(gray_img):
    ...
    G_x = cv2.filter2D(blur, -1, Kx).astype('int64')
    G_y = cv2.filter2D(blur, -1, Ky).astype('int64')
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    num_rows, num_cols, num = img_d.shape
    img_r = np.reshape(img_d,(num_rows*num_cols,3))
    ...
    cluster_img = np.reshape(clusters,(num_rows,num_cols,1))
...
def segment_image(img): 
    ...
    #binary = threshold_segment_naive(to_grayscale(img), 0, 200).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    #test_thresh_naive(test_img, 0, 220)
