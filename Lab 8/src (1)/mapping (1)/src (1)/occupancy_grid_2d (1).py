...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            fixedAngle = yaw - msg.angle_min + idx*msg.angle_increment
            ...
            walk = np.arange(r,0,-0.01)
            lastWalked = (None,None)
            for i in walk:
                fixed_x = sensor_x + i*math.cos(fixedAngle)
                fixed_y = sensor_y + i*math.sin(fixedAngle)
                (g_x,g_y) = self.PointToVoxel(fixed_x,fixed_y)
                if lastWalked == (None,None):
                    self._map[g_x,g_y] = self._map[g_x,g_y] + self._occupied_update
                    if self._map[g_x,g_y] > self._occupied_threshold:
                        self._map[g_x,g_y] = self._occupied_threshold
                    lastWalked = (g_x,g_y)
                elif lastWalked != (g_x,g_y):
                    self._map[g_x,g_y] = self._map[g_x,g_y] + self._free_update
                    if self._map[g_x,g_y] < self._free_threshold:
                        self._map[g_x,g_y] = self._free_threshold
                    lastwalked = (g_x,g_y)
