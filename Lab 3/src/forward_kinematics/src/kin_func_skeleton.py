...
def rotation_3d(omega, theta):
    ...
    return np.identity(3) + (omega_hat/magnitude)*np.sin(magnitude*theta) + (np.matmul(skew_3d(omega),skew_3d(omega))/(magnitude**2))*(1-np.cos(magnitude*theta))
...
def homog_3d(xi, theta):
    ...
    if (omega[0]==np.array([0,0,0])).all():
        ...
        g = np.zeros((4,4))
        g[0:3,0:3] = rotation_3d(omega[0], theta)
        g[3,:] = [0,0,0,1]
        omega_hat = skew_3d(omega[0])
        magnitude = np.linalg.norm(omega[0])
        g[0:3,3:] = (np.matmul(np.identity(3)-rotation_3d(omega[0],theta),(np.matmul(omega_hat,np.transpose(v))))+np.matmul(np.matmul(np.transpose(omega),omega),np.transpose(v))*theta)/(magnitude**2)
        return g
...
def prod_exp(xi, theta):
    ...
    for x in range(1,theta.size):
        g = np.matmul(g,homog_3d(xi[:,x],theta[x]))
    return g
